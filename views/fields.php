<?php 
	...
	elseif ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'profile_cct_toggle_field' ) ):
		...
		$field = isset($global_settings['clone_fields'][$field_index]) ? $global_settings['clone_fields'][$field_index] : '';
...
<pre>
<?php if(isset($local_clone_fields)){print_r( $local_clone_fields );} ?>
</pre>
	...
	<table class="form-table">
		<tr valign="top">
			...
			<td<?php if (isset($error['label'])) { echo ' class="form-invalid "';}?>>
				<input type="text" value="<?php echo esc_attr(isset($field_label) ? $field_label : ''); ?>" id="label" name="label" class="all-options"  /> <span class="description">For example: Lab Phone</span>
				<br />
			...
			<td<?php if (isset($error['field_clone'])) { echo ' class="form-invalid "';}?>>
				<select name="field_clone" id="field_clone" class="all-options">
					<?php foreach(Profile_CCT_Admin::fields_to_clone() as $field_to_clone): ?>
						<option value="<?php echo esc_attr($field_to_clone['type']);?>" <?php selected(isset($field_clone) ? $field_clone : 'phone', $field_to_clone['type']); ?>><?php echo esc_attr($field_to_clone['type']);?></option>
					<?php endforeach; ?>
				</select>
		...
		<tr valign="top">
			...
			<td<?php if (isset($error['description'])) { echo ' class="form-invalid "';}?>>
				<textarea name="description" id="description" class="large-text" cols="30" rows="5"><?php echo esc_textarea(isset($field_description) ? $field_description : ''); ?></textarea>
				<br />
